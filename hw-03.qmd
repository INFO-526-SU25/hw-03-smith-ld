...
```{r}
pkgs = c("tidyverse", "grid", "gridExtra", "ggplot2", "reshape", "ggalt")
for (pkg in pkgs) {
...
color_vals = c("Rent" = "#141512", "Food" = "#805D7D", "Clothes" = "#DA907F", "Tax"="#958c82", "Other"="#BDB39C")
...
ggplot(csss |> filter(csss[["response"]] == "resp_safety"), aes(x=response, y=interaction(explanatory, explanatory_value), group=explanatory)) +
  geom_segment(aes(x=low, y=explanatory_value, xend=high, yend=explanatory_value)) + 
  geom_point(aes(x=mean)) + 
  theme_minimal(
    
  ) + 
  labs(title="Based on my understanding")
```

```{r}
csss <- covid_survey_summary_stats
ggplot(csss |> filter(csss[["response"]] == "resp_safety"), aes(y=interaction(explanatory, explanatory_value, lex.order=TRUE), group=explanatory)) +
  geom_segment(aes(x=low, xend=high)) + 
  theme(
    legend.position=NULL,
    axis.
  ) + 
    geom_point(aes(x=low)) + geom_point(aes(x=high)) + geom_point(aes(x=mean)) + 
  labs(title="Based on my understanding", x="", y="")


#ggplot(df, aes(y = reorder(company, customers))) +
 # geom_segment(aes(x = customers, xend = potential_customers, yend = company)) +
#  geom_point(aes(x = customers), size = 3, color = "red") +
#  geom_point(aes(x = potential_customers), size = 3) 
